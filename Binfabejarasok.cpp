...
public:
   ...
   */
    void prekiir (void)
    {
        ...
        prekiir (&gyoker, std::cout);
    }
	  void postkiir (void)
    {
        melyseg = 0;
        postkiir (&gyoker, std::cout);
    }

	  void inkiir (void)
    {
        melyseg = 0;
        inkiir (&gyoker, std::cout);
    }
  ...
  */
    ...
    {
	os<<"Preorder bejarasi mod:\n";
        bf.prekiir (os);
	os<<"Postorder bejarasi mod:\n";
	bf.postkiir (os);
	os<<"Inorder bejarasi mod:\n";
	bf.inkiir (os);
        return os;
    }
...
//-----------------------------------------------------------
    ...
    void prekiir (std::ostream & os)
    {
        melyseg = 0;
        prekiir (&gyoker, os);
    }

	void postkiir (std::ostream & os)
    {
        melyseg = 0;
        postkiir (&gyoker, os);
    }
	void inkiir (std::ostream & os)
    {
        melyseg = 0;
        inkiir (&gyoker, os);
    }
...
private:
	...
	//void postkiir (Csomopont * elem, std::ostream & os);
    ...
    void inkiir (Csomopont * elem, std::ostream & os)
    {
	//inorder
        ...
        {
            ++melyseg;
            inkiir (elem->egyesGyermek (), os);
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            inkiir (elem->nullasGyermek (), os);
            --melyseg;
        }
    }
void prekiir (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
	//preorder
        if (elem != NULL)
        {
		 ...
		 for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;            
		prekiir (elem->egyesGyermek (), os);
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
           
            prekiir (elem->nullasGyermek (), os);
            --melyseg;
        }
    }
void postkiir (Csomopont * elem, std::ostream & os)
	...
	//postorder
		...
		postkiir (elem->egyesGyermek (), os);
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
           
            postkiir (elem->nullasGyermek (), os);
	 for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl; 
            --melyseg;
...
const char * inorderfile=fajlok[0];
//std::cout<<preorderfile<<std::endl;*/
std::fstream pre (*++argv, std::ios_base::out);
std::fstream in (*++argv, std::ios_base::out);
std::fstream post (*++argv, std::ios_base::out);    
while (beFile.read ((char *) &b, sizeof (unsigned char)))
    {
